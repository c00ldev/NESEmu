...
NROM::PRG_t::PRG_t(NROM & nrom)
	: nrom(nrom)
{
}

uint8_t NROM::PRG_t::read(uint16_t addr) const
{
	auto [pMem, pAddr] = resolve(addr);
	return pMem.read(pAddr);
}

uint8_t NROM::PRG_t::write(uint16_t addr, uint8_t val)
{
	auto [pMem, pAddr] = resolve(addr);
	return pMem.write(pAddr, val);
}

uint8_t NROM::PRG_t::write(uint16_t addr, uint8_t val) const
{
	auto [pMem, pAddr] = resolve(addr);
	return pMem.write(pAddr, val);
}

size_t NROM::PRG_t::size() const
{
	return 0xA000;
}

std::pair<Memory &, uint16_t> NROM::PRG_t::resolve(uint16_t addr)
{
	auto [pMem, pAddr] = static_cast<const NROM &>(nrom).PRG.resolve(addr);
	return std::pair(std::ref(const_cast<Memory &>(pMem)), pAddr);
}

std::pair<const Memory &, uint16_t> NROM::PRG_t::resolve(uint16_t addr) const
{
